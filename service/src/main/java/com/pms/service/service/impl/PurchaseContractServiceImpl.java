...
//            Map<String, Object> map = this.dao.findOne(ApiConstants.MONGO_ID, purchaseContractId, new String[] { ProjectBean.PROJECT_TYPE }, DBBean.PROJECT);
            ...
            if (dao.exist("projectId", params.get("projectId"), DBBean.PURCHASE_CONTRACT)) {
				...
				if (ApiUtil.isValid(params.get(PurchaseRequest.SALES_CONTRACT_ID))) {
				    ...
				    //query null data
				    query.put(PurchaseRequest.SALES_CONTRACT_ID, "");
				}
...
//            query.put(PurchaseContract.SUPPLIER_ID, params.get(PurchaseContract.SUPPLIER_ID));
        ...
        for (Map<String, Object> data : list) {
            ...
            for (Map<String, Object> eq : eqCostList) {
                ...
                if (isDirect) {
                    if (eq.get(PurchaseRequest.PROJECT_ID) != null && eq.get(PurchaseRequest.PROJECT_ID).equals(params.get(PurchaseRequest.PROJECT_ID)) && 
        ...
        if(db.equalsIgnoreCase(DBBean.REPOSITORY)){
        	requery.put(PurchaseContract.PURCHASE_CONTRACT_ID, params.get(PurchaseContract.PURCHASE_CONTRACT_ID));
...
//                    createAutoShip(contract);
    ...
    public Map<String, Object> backPurchaseOrderToSc(Map<String, Object> parameters){       
        ...
        Map<String, Object> request = new HashMap<String, Object>();
        request.put(ApiConstants.MONGO_ID, order.get(PurchaseRequest.PURCHASE_REQUEST_ID ));   
        backPurchaseRequestToSc(request);
    ...
    public Map<String, Object> backPurchaseRequest(Map<String, Object> parameters) {
        ...
        backService.updatePurchaseBackStatus(request.get(PurchaseCommonBean.BACK_REQUEST_ID).toString());
        return parameters;        
    }
    
	public Map<String, Object> backPurchaseRequestToSc(Map<String, Object> parameters) {
		processRequest(parameters, DBBean.PURCHASE_REQUEST, PurchaseRequest.STATUS_BACKED);
    ...
    public Map<String, Object> getRepositoryRequest(Map<String, Object> parameters) {
        ...
        result.put(SalesContractBean.SC_EQ_LIST, scs.mergeEqListBasicInfo(mergedEqList));
	...
	public Map<String, Object> updateRepositoryRequest(Map<String, Object> parameters) {
		...
		if (parameters.get(ApiConstants.MONGO_ID) == null && ApiUtil.isEmpty(parameters.get("repositoryCode"))) {
			if ("out".equalsIgnoreCase((String) parameters.get("type"))) {
    ...
    public Map<String, Object> approveRepositoryRequest(Map<String, Object> params) {
        ...
        if (params.get("type") != null && params.get("type").toString().equalsIgnoreCase("in")) {
			...
			for (Map<String, Object> eqMap : eqMapList) {
				int eqAmount = ApiUtil.getInteger(eqMap.get(PurchaseRequest.EQCOST_APPLY_AMOUNT), 0);
				int confirmAmount = ApiUtil.getInteger(eqMap.get("eqcostConfirmApplyAmount"), 0);
				int confirmedAmount = ApiUtil.getInteger(eqMap.get("eqcostConfirmedAmount"), 0);
        ...
        try {
            ...
            for (int i = n + 1; i < list.size(); i++) {// 从第5行开始读数据
                ...
                if(ApiUtil.isEmpty(signBy)){
                	if(ApiUtil.isValid(columnIndexMap.get("签订人"))){
                ...
                contract.put("eqcostDeliveryType", "入公司库");
                ...
                String contractCode = item[columnIndexMap.get("销售合同编号")].trim();
...
//                    projectService.importProject(project);
                ...
                contract.put(PurchaseCommonBean.SALES_CONTRACT_CODE, contractCode);
                ...
                if (order != null && false) {
                    ...
                    for (Map<String, Object> eqMap : eqList) {
                        ...
                        eqMap.put("eqcostDeliveryType", PurchaseRequest.EQCOST_DELIVERY_TYPE_REPOSITORY);
                        eqMap.put("contractExecuteCate", PurchaseRequest.CONTRACT_EXECUTE_CATE_NORMAL);

                        eqMap.put(PurchaseContract.PURCHASE_CONTRACT_ID, contract.get(ApiConstants.MONGO_ID));
                        eqMap.put(PurchaseContract.PURCHASE_CONTRACT_CODE, contract.get(PurchaseContract.PURCHASE_CONTRACT_CODE));
                        eqMap.put(PurchaseContract.SUPPLIER_ID, contract.get(PurchaseContract.SUPPLIER_ID));
                        eqMap.put(PurchaseContract.SUPPLIER_NAME, contract.get(PurchaseContract.SUPPLIER_NAME));
                        eqMap.put(PurchaseContract.PURCHASE_CONTRACT_TYPE, contract.get(PurchaseContract.PURCHASE_CONTRACT_TYPE));
    ...
    public void backContractToSc(Map<String, Object> parameters) {
        processRequest(parameters, DBBean.PURCHASE_CONTRACT, PurchaseRequest.STATUS_CANCELLED);

        Map<String, Object> query = new HashMap<String, Object>();
        query.put(PurchaseContract.PURCHASE_CONTRACT_ID, parameters.get(ApiConstants.MONGO_ID));
